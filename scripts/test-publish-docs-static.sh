...
create_fixture_repo() {
  ...
  echo "static/" > "${work_repo}/.gitignore"
  git -C "${work_repo}" add README.md .gitignore scripts/publish-docs-static.sh
  ...
  mkdir -p "${work_repo}/static/api/1.2.2"
  mkdir -p "${work_repo}/static/api/1.2.3"
  mkdir -p "${work_repo}/static/api/snapshot"
  mkdir -p "${work_repo}/static/demo/1.2.2"
  mkdir -p "${work_repo}/static/demo/1.2.3"
  mkdir -p "${work_repo}/static/demo/snapshot"
  mkdir -p "${work_repo}/static/playground/1.2.2"
  mkdir -p "${work_repo}/static/playground/1.2.3"
  mkdir -p "${work_repo}/static/playground/snapshot"

  echo "preserve" > "${work_repo}/static/demo/1.2.2/keep.txt"
  echo "preserve" > "${work_repo}/static/api/1.2.2/keep.txt"
  echo "preserve" > "${work_repo}/static/playground/1.2.2/keep.txt"
  echo "stale-current" > "${work_repo}/static/demo/1.2.3/stale.txt"
  echo "stale-current" > "${work_repo}/static/api/1.2.3/stale.txt"
  echo "stale-current" > "${work_repo}/static/playground/1.2.3/stale.txt"
  echo "stale-snapshot" > "${work_repo}/static/demo/snapshot/stale.txt"
  echo "stale-snapshot" > "${work_repo}/static/api/snapshot/stale.txt"
  echo "stale-snapshot" > "${work_repo}/static/playground/snapshot/stale.txt"

  git -C "${work_repo}" add -f static
  ...
  rm -rf "${work_repo}/static"
  mkdir -p "${work_repo}/static/api/1.2.3"
  mkdir -p "${work_repo}/static/demo/1.2.3"
  mkdir -p "${work_repo}/static/playground/1.2.3"

  echo "new-current" > "${work_repo}/static/demo/1.2.3/new.txt"
  echo "new-current" > "${work_repo}/static/api/1.2.3/new.txt"
  echo "new-current" > "${work_repo}/static/playground/1.2.3/new.txt"
  ...
  if [[ "${mode}" == "release" ]]; then
    ...
    mkdir -p "${work_repo}/static/demo/snapshot"
    mkdir -p "${work_repo}/static/playground/snapshot"
    echo "should-not-publish" > "${work_repo}/static/demo/snapshot/new.txt"
    echo "should-not-publish" > "${work_repo}/static/api/snapshot/new.txt"
    echo "should-not-publish" > "${work_repo}/static/playground/snapshot/new.txt"
  else
    # Snapshot run should fully refresh snapshot.
    mkdir -p "${work_repo}/static/api/snapshot"
    mkdir -p "${work_repo}/static/demo/snapshot"
    mkdir -p "${work_repo}/static/playground/snapshot"
    echo "new-snapshot" > "${work_repo}/static/demo/snapshot/new.txt"
    echo "new-snapshot" > "${work_repo}/static/api/snapshot/new.txt"
    echo "new-snapshot" > "${work_repo}/static/playground/snapshot/new.txt"
  fi
...
test_release_refreshes_current_and_preserves_snapshot_history() {
  ...
  assert_file_exists \
    "${inspect_repo}/static/demo/1.2.2/keep.txt" \
    "preserves older demo version"
  assert_file_exists \
    "${inspect_repo}/static/api/1.2.2/keep.txt" \
    "preserves older api version"
  assert_file_exists \
    "${inspect_repo}/static/playground/1.2.2/keep.txt" \
    "preserves older playground version"

  assert_file_exists \
    "${inspect_repo}/static/demo/1.2.3/new.txt" \
    "refreshes current demo version"
  assert_file_exists \
    "${inspect_repo}/static/api/1.2.3/new.txt" \
    "refreshes current api version"
  assert_file_exists \
    "${inspect_repo}/static/playground/1.2.3/new.txt" \
    "refreshes current playground version"

  assert_file_missing \
    "${inspect_repo}/static/demo/1.2.3/stale.txt" \
    "removes stale file from current demo version"
  assert_file_missing \
    "${inspect_repo}/static/api/1.2.3/stale.txt" \
    "removes stale file from current api version"
  assert_file_missing \
    "${inspect_repo}/static/playground/1.2.3/stale.txt" \
    "removes stale file from current playground version"

  assert_file_exists \
    "${inspect_repo}/static/demo/snapshot/stale.txt" \
    "keeps snapshot demo unchanged on release"
  assert_file_exists \
    "${inspect_repo}/static/api/snapshot/stale.txt" \
    "keeps snapshot api unchanged on release"
  assert_file_exists \
    "${inspect_repo}/static/playground/snapshot/stale.txt" \
    "keeps snapshot playground unchanged on release"

  assert_file_missing \
    "${inspect_repo}/static/demo/snapshot/new.txt" \
    "does not publish snapshot demo changes on release"
  assert_file_missing \
    "${inspect_repo}/static/api/snapshot/new.txt" \
    "does not publish snapshot api changes on release"
  assert_file_missing \
    "${inspect_repo}/static/playground/snapshot/new.txt" \
