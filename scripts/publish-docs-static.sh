...
if [[ ! -d "${STATIC_DIR}" ]]; then
  echo "static is missing. Generate docs static artifacts before publishing." >&2
  exit 1
fi

if ! command -v rsync >/dev/null 2>&1; then
  echo "rsync is required to publish static." >&2
  exit 1
fi
...
mkdir -p "${WORKTREE}/static"

refresh_subdir() {
  local rel_path="$1"
  local src="${STATIC_DIR}/${rel_path}"
  local dst="${WORKTREE}/static/${rel_path}"
...
if [[ "${IS_SNAPSHOT_VERSION}" == "true" ]]; then
  rsync -a "${STATIC_DIR}/" "${WORKTREE}/static/"
else
  rsync -a \
    --exclude "api/snapshot/" \
    --exclude "demo/snapshot/" \
    --exclude "playground/snapshot/" \
    "${STATIC_DIR}/" "${WORKTREE}/static/"
fi

git -C "${WORKTREE}" add -f static
