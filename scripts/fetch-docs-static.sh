...
DOCS_APP_PUBLIC_DIR="${REPO_ROOT}/docs-app/public"
...
if ! git -C "${REPO_ROOT}" rev-parse --is-inside-work-tree >/dev/null 2>&1; then
  echo "Not a git repository; skipping static fetch." >&2
  sync_public_assets
  exit 0
fi
...
if ! git -C "${REPO_ROOT}" remote get-url "${REMOTE}" >/dev/null 2>&1; then
  ...
  if [[ -n "${REMOTE_URL}" ]]; then
    ...
    echo "Remote ${REMOTE} not configured; skipping static fetch." >&2
    sync_public_assets
    exit 0
  fi
fi
